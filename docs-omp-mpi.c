...
#pragma omp parallel private(token,id_temp,id_chunk,document,i) shared(buffer) if(num_documents*num_subjects*8 < 1000000)
	while(token != NULL) {
...
int move_documents()
{
  ...
  if(proc_id) {
#pragma omp parallel for private(i,j,k,shorty,shortest,dist, coord, document, cabinet) shared(changed)
    for(i = 0; i < num_docs_chunk; i++) {
      shortest = DBL_MAX;
